...
public class PushEventTest {
    ...
    /**
     * First 2 Pull Requests have {@link io.xstefank.wildfly.bot.model.RuntimeConstants#LABEL_NEEDS_REBASE} applied and
        ...
        given().github(mocks -> {
            ...
            when(pullRequestsPagedIterable.toList()).thenReturn(pullRequests);
                ...
                .then().github(mocks -> {
                    ...
                    verify(mocks.repository(TEST_REPO)).queryPullRequests();
                    for (int i = 0; i < pullRequestCount; i++) {
                    ...
                    verify(mocks.repository(TEST_REPO)).getDirectoryContent(anyString());
                    Mockito.verifyNoMoreInteractions(mocks.repository(TEST_REPO));
                });
    }
