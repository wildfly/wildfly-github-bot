...
public final class PullRequestDescriptionHandler {
    ...
    public PullRequestDescriptionHandler(GHPullRequest pullRequest, Pattern issueKeyPattern) {
        if (pullRequest == null || issueKeyPattern == null) {
            throw new IllegalArgumentException("Pull request and issue link pattern must not be null");
        }
        this.gitHubAppName = Arc.container().instance(GitHubBotContextProvider.class).get().getBotName();
    ...
    public Optional<String> generateFullDescriptionBody() {
        ...
        if (newBotBody.toString().equals(botBody)) { // no changes in the bot body
    ...
    private StringBuilder generateBotBodySection() {
        final StringBuilder botBodySection = new StringBuilder();
