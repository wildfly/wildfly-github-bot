 ...
 * see {@see io.xstefank.wildfly.bot.config.WildFlyBotConfig#timeout()}
 * <p>
 ...
 * due to possible race condition of the {@code io.xstefank.wildfly.bot.util.PullRequestLogger}.
 */
@Singleton
public class PullRequestMergableProcessor {
    ...
    public void addPushPayload(GHEventPayload.Push pushPayload) {
        ...
        Uni<List<GHPullRequest>> mergableStatusUpdateUni = Uni.createFrom()
                ...
                .item(repository.queryPullRequests().state(GHIssueState.OPEN).base(MAIN_BRANCH).list())
                .invoke(() -> LOGGER.infof(
    ...
    private void logResult(List<GHPullRequest> ghPullRequests, GHEventPayload.Push.PushCommit headCommit) {
        ...
        if (!unknownPullRequests.isEmpty()) {
            ...
            LOGGER.warnf(
                    ...
                    MAIN_BRANCH_REF, headCommit.getSha(), headCommit.getMessage(), concatenatedPRs);
        } else {
            LOGGER.infof(
                    "Successfully scanned all pull requests for new %s branch head [%s - \"%s\"] and updated '%s' label accordingly. ",
                    MAIN_BRANCH_REF, headCommit.getSha(), headCommit.getMessage(), LABEL_NEEDS_REBASE);
        }
    }
